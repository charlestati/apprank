...
- Migrations `0001_better_auth.sql` and `0004_drop_session_tables.sql` create a set of session tables and then drop them. That pair is why: a session-based auth library was tried and removed, because it took the web Worker from 84 KiB to 1,939 KiB to serve a handful of accounts that never change. Migration history is append-only, so both files stay. Do not reach for one again unless self-service signup becomes a real requirement — basic auth over a secret does the job in a few dozen lines.
