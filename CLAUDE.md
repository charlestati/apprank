...
- **Three rules keep the crawl affordable, and all three exist because the naive version was expensive.** (1) `pickDuePair` will not return a pair that already has a `ranking` row for today — the table is unique on `(pair_id, observed_date)`, so a same-day re-crawl overwrites its own row and buys nothing. Anything that moves `next_due_at` backwards (a reseed, a manual re-run) would otherwise re-crawl the whole set at ~30 writes a pair. (2) The `app` upsert carries a `WHERE`, because `last_seen_at = now` always differs and made every top-10 app on every pair a guaranteed daily write for a column nothing reads. (3) `rank_entry` is only rewritten when the indexed page actually moved; a board that held its order used to cost a delete plus eleven inserts to arrive at the same rows.
