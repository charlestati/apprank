...
Metadata changes are drawn on the rank chart as markers, because a rank move is only interpretable against the release that might have caused it.
...
- `@cloudflare/workers-types` v5 dropped versioned entrypoints. Types come from `wrangler types` (generated `worker-configuration.d.ts`, gitignored).
