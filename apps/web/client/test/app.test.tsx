...
describe("App shell", () => {
  ...
  it("goes green once every Tier-1 pair has been collected", async () => {
    ...
    expect(status()?.textContent).toContain("10/10 today");
  ...
  it("goes critical and singularises a lone error", async () => {
    stubFetch({
      ...
      "health/data": dataHealth({
        collectedToday: 10,
        errorsLast24h: [
          { errorClass: "throttled", lastAt: 0, message: null, n: 1 },
        ],
        tier1Pairs: 10,
      }),
    });
  ...
  it("pluralises several errors", async () => {
    stubFetch({
      ...
      "health/data": dataHealth({
        errorsLast24h: [
          { errorClass: "throttled", lastAt: 0, message: null, n: 2 },
          { errorClass: "http_error", lastAt: 0, message: null, n: 1 },
        ],
      }),
    });
    render(<App />);
  ...
  it("shows zero coverage without a divide-by-zero when nothing is due", async () => {
    ...
    render(<App />);
    await expect(screen.findByText("Collecting")).resolves.toBeDefined();
    expect(status()?.textContent).toContain("0/0 today");
  });
