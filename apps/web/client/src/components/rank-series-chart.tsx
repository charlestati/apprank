...
interface Props {
	...
	/** Release the reader is pointing at, by date; the other pins grey out. */
...
/**
 * Pointing at a release greys the others and brings that one forward. Both
 * halves are needed: dimming alone does nothing when there is only one release,
 * which is the common case for a young install.
 */
function pinClass(base: string, state: "on" | "off" | "idle"): string {
	if (state === "off") {
		return `${base} marker-muted`;
	}
	return state === "on" ? `${base} marker-on` : base;
}
...
/**
 * The day the axis actually starts, when that is later than the day the window
 * asked for; null when the window is fully covered. History cannot be
 * backfilled, so on a young install this is simply the day collection began.
 */
function laterStart(
	from: string | undefined,
	axis: string | undefined
): string | null {
	return from && axis && axis > from ? axis : null;
}

/**
 * Dot radius: a lone observation is drawn heavier, and marks that land on the
 * same point are drawn as concentric rings so none of them is hidden under
 * another. The rings are hollow, so their paint order does not matter.
 */
function dotRadius(pointsInSegment: number, depth: number): number {
	return (pointsInSegment === 1 ? 4 : 3) + depth * 3;
}
...
export function RankSeriesChart({
	...
	const dates = useMemo(() => {
		...
		const [first] = sorted;
		if (bounds) {
			// The axis stays a calendar, one slot per day, but it does not begin
			// before collection did. A fortnight-old install asked to draw ninety
			// days spent eighty-eight of them on empty rungs and packed every
			// observation into the last few pixels, where a series with a single
			// point is one dot against the right edge and reads as a broken chart.
			// Only the start moves: a trailing empty stretch means collection
			// stopped, which is exactly what has to stay visible.
			return calendarDays(
				laterStart(bounds.from, first) ?? bounds.from,
				bounds.to
			);
		}
		if (first === undefined) {
		...
		return calendarDays(first, sorted.at(-1) as string);
	}, [series, bounds]);
	...
	const { maxRank, x, y, lines } = useMemo(() => {
		...
		const built = series.map((s) => {
			...
			for (const [date, i] of byDate) {
				...
				if (pos === undefined || pos === null) {
					...
					current.push({ depth: 0, x: xOf(i), y: yOf(pos) });
				}
			}
			if (current.length > 0) {
				segments.push({ points: current });
		...
		// Four keywords at rank 1 on the same day are four dots on one pixel, and
		// only the last one painted is ever seen. Marks that share a point are
		// drawn as rings around it rather than nudged apart: a nudge would state a
		// date or a rank the data does not have. Depth is counted across every
		// series, hidden or greyed, so the rings do not resize under the pointer.
	...
	// Non-null only when the window asked for days that predate collection.
		...
		<div className="chart" ref={measure}>
            ...
            has to be able to tell "nothing failed" from "failures not shown". */}
				...
				{markers.map((marker, n) => {
					...
					// A tag says "release" where a bare numeral said "footnote", and the
					// numeral beside it is what keys the pin to its line in the caption:
					// three releases in a fortnight are otherwise indistinguishable, and
					// a marker you cannot attribute anchors nothing.
					// The same contrast the series legend uses, so the two keys behave
					// alike.
					let state: "on" | "off" | "idle" = "idle";
					if (focusedMarker !== null) {
						state = focusedMarker === marker.date ? "on" : "off";
					}
					return i === -1 ? null : (
						<g key={marker.date}>
							<line
								className={pinClass("marker-line", state)}
								x1={x(i)}
								x2={x(i)}
								y1={MARKER_ROW + 8}
								y2={PLOT_BOTTOM}
							/>
							<Tag
								className={pinClass("marker-pin", state)}
								height={MARKER_SIZE}
								width={MARKER_SIZE}
								x={x(i) - MARKER_SIZE}
								y={MARKER_ROW - MARKER_SIZE / 2}
							>
								<title>{markerTitle(marker, t, f.day)}</title>
							</Tag>
							<text
								className={pinClass("marker-index", state)}
								textAnchor="start"
								x={x(i) + 2}
								y={MARKER_ROW + 4}
							>
				...
				{lines.map(({ row, segments, unranked, failures }) => {
					...
					return (
						...
						>
							{segments.map((seg, i) => (
								<g key={`${row.pairId}-${i}`}>
									{/* One observation is not a line. A two-coordinate polyline
                      is what SVG needs before it paints anything, so a lone
                      day, which is every day of a freshly tracked keyword,
                      would otherwise be drawn as nothing at all: its dot
                      carries the whole series and is sized to say so. */}
									{seg.points.length > 1 ? (
										<polyline
									...
									{muted
										...
										: seg.points.map((p) => (
												<circle
													className={
														p.depth > 0 ? "series-dot-ring" : "series-dot"
													}
													cx={p.x}
													cy={p.y}
													fill={p.depth > 0 ? "none" : stroke}
													key={`${p.x}-${p.y}`}
													r={dotRadius(seg.points.length, p.depth)}
													stroke={p.depth > 0 ? stroke : undefined}
												/>
											))}
								</g>
							))}
							...
							{failures.map(({ i, errorClass }) => (
								// Clamped to the plot. A slot is the full width of a day, so
								// on a four-day axis it is some 270px, and the first one
								// centred on its tick started ninety pixels off-canvas and
								// drew straight through the rail's own label.
								<rect
									...
									key={`e-${i}`}
									width={
										Math.min(x(i) + slot / 2, w - PAD.right) -
										Math.max(PAD.left, x(i) - slot / 2)
									}
									x={Math.max(PAD.left, x(i) - slot / 2)}
									y={errorRailY - 4.5}
								>
