...
describe("GET /api/health/data", () => {
	...
	it("summarises tier-1 pairs, today's collection, errors, pacing and anomalies", async () => {
		...
		// Grouped by endpoint as well as class: two throttles on one endpoint and
		// one on another are three different facts, and a class on its own names
		// no subject.
		expect(
			body.errorsLast24h.map((e) => ({
				endpoint: e.endpoint,
				errorClass: e.errorClass,
				n: e.n,
			}))
		).toStrictEqual([
			{ endpoint: "/search", errorClass: "throttled", n: 2 },
			{ endpoint: "/search", errorClass: "timeout", n: 1 },
		]);
