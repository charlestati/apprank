...
Passwords are compared as SHA-256 digests in constant time. That is deliberate rather than an oversight: a KDF would be the most expensive thing in the request on the free tier's 10 ms CPU budget, and these are machine-generated high-entropy secrets, so there is no offline brute-force to slow down.
