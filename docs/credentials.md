...
Every credential is a Worker secret, and the apps to collect for come from the
`tracked_app` table.

| Name                                                            | Lives on                                                    | Purpose                                                                                                                                                                                             |
| --------------------------------------------------------------- | ----------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `BASIC_AUTH_ACCOUNTS`                                           | `apps/web` Worker                                           | The wall in front of the whole origin. A JSON array of accounts, listed in [access control](access.md). Without it the Worker serves nothing at all, deliberately: it fails closed rather than open |
| `ADMIN_TOKEN`                                                   | `apps/collector` Worker, `.dev.vars`, GitHub                | Gates `POST /admin/run?job=…`, the collector's only public route. Not a debug convenience: every collection run drives the collector through it, so no token means no collection                    |
| `ASC_ISSUER_ID`, `ASC_KEY_ID`, `ASC_PRIVATE_KEY`                | `apps/collector` Worker                                     | App Store Connect analytics. Optional; the daily job skips them quietly when absent                                                                                                                 |
| `ADS_CLIENT_ID`, `ADS_TEAM_ID`, `ADS_KEY_ID`, `ADS_PRIVATE_KEY` | `apps/collector` Worker                                     | Apple Ads search-term popularity, the only official volume figure Apple gives, though it covers only head terms. Optional in the same way                                                           |
| `MCP_ENABLED`                                                   | `apps/web/wrangler.local.jsonc` (a var, not a secret)       | Off unless set to `true`. The MCP endpoint at `/mcp` is opt-in: without it the route 404s and nothing is published                                                                                  |
| `COLLECTION_MODE`                                               | `apps/collector/wrangler.local.jsonc` (a var, not a secret) | `all` by default. Set it to `credentialed` on a deployment whose egress Apple rejects, which is every Cloudflare Worker                                                                             |
...
```sh
openssl rand -base64 24        # generate the new value
cd apps/collector

# 1. the deployed Worker
npx wrangler secret put ADMIN_TOKEN -c wrangler.local.jsonc

# 2. local runs: wrangler dev and refresh.sh both read this file
printf 'ADMIN_TOKEN=%s\n' 'THE_NEW_VALUE' > .dev.vars

# 3. GitHub, by hand: Settings → Secrets and variables → Actions
```
...
**Decide whether it is worth it first.** Apple publishes popularity for roughly
the top 500 terms per _top-level_ category per storefront, so a specific
long-tail phrase competing inside GAMES or PRODUCTIVITY usually returns nothing.
On the tracked set here, 4 of 25 keywords came back with a number; the rest are
stored `present = 0`, meaning asked and unpublished. Head terms fare much
better. Everything else in the report works without this, and keywords with no
published volume are judged on rank and difficulty alone.

Two traps below, either of which will cost you an evening.
